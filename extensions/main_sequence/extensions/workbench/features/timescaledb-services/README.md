...
- `GET /orm/api/connections/timescaledb-service/{uid}/` for the detail payload.
- `GET /orm/api/connections/timescaledb-service/{uid}/summary/` for the standardized summary header.
- `GET /orm/api/connections/timescaledb-service/{uid}/data-sources/` for the nested physical-database registry.
- `GET /orm/api/connections/timescaledb-service/{uid}/runtime-config/` for runtime configuration details.
- `GET /orm/api/connections/timescaledb-service/{uid}/changes/` for service change history.
- `POST /orm/api/connections/timescaledb-service/{uid}/request-storage-increase/` for storage increase requests.

## Notes

- This feature is intentionally read-only because the backend viewset blocks create, update, and delete operations.
- Detail routing is URL-backed with `msTimeScaleDbServiceUid` and `msTimeScaleDbServiceTab` so selected services and tabs are linkable and refresh-safe.
