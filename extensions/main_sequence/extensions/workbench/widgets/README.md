...
registers the Data Node, data-node graph, data-node statistic, and data-node table widgets in the
live widget catalog.
...
- `data-node-visualizer/`, `data-node-filter/`, `data-node-statistic/`, and `data-node-table/` are
  currently registered in the app-wide widget catalog from
