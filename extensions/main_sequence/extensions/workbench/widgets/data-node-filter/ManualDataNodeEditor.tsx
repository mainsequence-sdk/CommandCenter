...
import type { ManualDataNodeColumnDefinition } from "../data-node-shared/dataNodeWidgetSource";

const agGridModules = [AllCommunityModule];

const manualColumnTypeOptions: Array<{
  label: string;
  value: ManualDataNodeColumnDefinition["type"];
}> = [
...
function buildEmptyRow(columns: readonly ManualDataNodeColumnDefinition[]) {
  return Object.fromEntries(columns.map((column) => [column.key, ""])) as Record<string, unknown>;
}

function buildManualGridRows(
  columns: readonly ManualDataNodeColumnDefinition[],
...
function inferImportedColumnType(values: readonly string[]): ManualDataNodeColumnDefinition["type"] {
...
function buildImportedManualTable(
  ...
  const nextColumns = headerCandidates.reduce<ManualDataNodeColumnDefinition[]>(
...
export function ManualDataNodeEditor({
  columns,
  editable,
  rows,
  onChange,
  ...
  const [draftColumns, setDraftColumns] = useState<ManualDataNodeColumnDefinition[]>(columns);
  ...
  function handleAddColumn() {
    const nextColumn: ManualDataNodeColumnDefinition = {
  ...
  function handleColumnTypeChange(
    columnIndex: number,
    nextType: ManualDataNodeColumnDefinition["type"],
  ) {
    setDraftColumns(
  ...
  return (
    <div className="space-y-3">
      ...
      <Dialog
        ...
        description="Edit the local Data Node table in a grid and import CSV or spreadsheet data. Apply commits these changes to the workspace draft."
            ...
            <section className="space-y-3 rounded-[calc(var(--radius)-8px)] border border-border/70 bg-background/24 p-4">
              <div className="space-y-1">
                ...
                <p className="text-sm text-muted-foreground">
                  Rename columns, choose types, and remove fields before publishing the manual dataset.
                </p>
              </div>
                        ...
                        <Select
                          ...
                          onChange={(event) => {
                            handleColumnTypeChange(
                              columnIndex,
                              event.target.value as ManualDataNodeColumnDefinition["type"],
                            );
                          }}
