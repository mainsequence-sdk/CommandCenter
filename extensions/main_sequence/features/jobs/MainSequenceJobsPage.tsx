...
export function MainSequenceJobsPage() {
  ...
  function openProjectDetail(projectId: number) {
    ...
    navigate(`/app/main_sequence/projects?${nextParams.toString()}`);
  }
  ...
  const jobSummary = selectedJob
    ? buildJobSummary(selectedJob, {
        ...
        projectHref: `/app/main_sequence/projects?${mainSequenceProjectIdParam}=${selectedJob.project}&${mainSequenceTabParam}=jobs`,
      })
    : null;
  const jobTitle =
    selectedJob?.name ??
