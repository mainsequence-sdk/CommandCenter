...
It is intentionally separate from the `Project Agents` configuration page so opening logs changes
...
- `ProjectAgentDeploymentLogsPage.tsx`
  ...
  summaries, and links back to the `Project Agents` surface while preserving `msProjectUid` when
  present.
...
- The endpoint is scoped to the authenticated user. Do not pass `created_by_user_uid` or legacy
  `created_by_user`.
- The current project UID is only preserved for navigation back to project-agent configuration.
