...
import {
  lightweightChartsHeatmapCells,
  lightweightChartsHeatmapColumns,
  lightweightChartsHeatmapRows,
  type LightweightChartsHeatmapCell,
} from "@/widgets/extensions/lightweight-charts/mock-data";
...
function formatIntervalLabel(cell: LightweightChartsHeatmapCell) {
...
export function HeatmapMatrixWidget({
  ...
  const strongestCell = useMemo(
    () =>
      lightweightChartsHeatmapCells.reduce((strongest, cell) =>
  ...
  useEffect(() => {
    const nextSelectionKey =
      runtimeSelectedKey &&
      lightweightChartsHeatmapCells.some((cell) => cell.key === runtimeSelectedKey)
        ? runtimeSelectedKey
        : strongestCell.key;
  ...
  const selectedCell = useMemo(
    () =>
      lightweightChartsHeatmapCells.find((cell) => cell.key === selectedCellKey) ?? strongestCell,
  ...
  const summary = useMemo(() => {
    const positiveCells = lightweightChartsHeatmapCells.filter((cell) => cell.signal > 0).length;
    const negativeCells = lightweightChartsHeatmapCells.length - positiveCells;
    const averageSignal =
      lightweightChartsHeatmapCells.reduce((total, cell) => total + cell.signal, 0) /
      lightweightChartsHeatmapCells.length;
  ...
  function handleSelectCell(cell: LightweightChartsHeatmapCell) {
    startTransition(() => {
  ...
  return (
    <div className="flex h-full min-h-0 flex-col gap-4">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="space-y-2">
          ...
          <div className="text-xs text-muted-foreground">
            Cross-asset pressure map with mocked regime scores and a lightweight-charts
            drilldown for the active cell.
          </div>
        </div>
      ...
      <div className="grid min-h-0 flex-1 grid-cols-[108px_repeat(6,minmax(0,1fr))] gap-1.5">
        <div />
        {lightweightChartsHeatmapColumns.map((column) => {
          ...
          return (
            <div
              ...
              style={{
                borderColor: withAlpha(active ? selectedTone : resolvedTokens.border, active ? 0.55 : 0.7),
        ...
        {lightweightChartsHeatmapRows.map((row) => (
          <div key={row} className="contents">
            ...
            {lightweightChartsHeatmapColumns.map((column) => {
              const cell = lightweightChartsHeatmapCells.find(
