...
type ColumnRange = ReturnType<typeof getTableWidgetColumnRange>;

interface TableWidgetCellRendererParams
  extends ICellRendererParams<TableWidgetRow, TableWidgetCellValue> {
...
function createTableCellStyle({
  ...
  range,
  tokens,
}: Omit<TableWidgetCellRendererParams, "api" | "colDef" | "column" | "context" | "data" | "eGridCell" | "eParentOfValue" | "getValue" | "node" | "pinned" | "registerRowDragger" | "refreshCell" | "setTooltip" | "valueFormatted">): CellStyle {
...
function TableWidgetCellRenderer({
  value,
  columnConfig,
  resolvedProps,
  range,
  tokens,
}: TableWidgetCellRendererParams) {
  const cellValue = value ?? null;
...
export function TableWidget({ props, instanceId, resolvedInputs }: Props) {
  ...
  const defaultColDef = useMemo<ColDef<TableWidgetRow>>(
    () => ({
      sortable: true,
      resizable: true,
      filter: true,
  ...
  if (isManualTableMode && resolvedProps.columns.length === 0) {
    return (
      <div className="flex h-full flex-col items-center justify-center gap-3 border border-dashed border-border/70 bg-background/35 px-4 py-6 text-center">
        ...
        <div className="space-y-1">
          <div className="text-sm font-medium text-foreground">Add manual table columns</div>
          <p className="text-sm text-muted-foreground">
            Open widget settings and add columns or paste rows into the manual table editor.
          </p>
        </div>
      </div>
    );
