import { FileText } from "lucide-react";

import { Badge } from "@/components/ui/badge";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { Select } from "@/components/ui/select";
import { Textarea } from "@/components/ui/textarea";
import { DEFAULT_WIDGET_SIZE } from "@/widgets/types";
import type { WidgetSettingsComponentProps } from "@/widgets/types";

import {
  normalizeRichTextNoteVerticalAlign,
  normalizeRichTextNoteWidth,
  RichTextNoteWidget,
  richTextNoteStarterHtml,
  type RichTextNoteWidgetProps,
} from "./RichTextNoteWidget";

export function RichTextNoteWidgetSettings({
  draftProps,
  editable,
  onDraftPropsChange,
}: WidgetSettingsComponentProps<RichTextNoteWidgetProps>) {
  const contentWidth = normalizeRichTextNoteWidth(draftProps.contentWidth);
  const contentVerticalAlign = normalizeRichTextNoteVerticalAlign(draftProps.contentVerticalAlign);
  const openLinksInNewTab = draftProps.openLinksInNewTab !== false;
  const previewProps: RichTextNoteWidgetProps = {
    ...draftProps,
    contentHtml: draftProps.contentHtml?.trim() || richTextNoteStarterHtml,
    emptyState: draftProps.emptyState?.trim() || undefined,
    contentWidth,
    contentVerticalAlign,
    openLinksInNewTab,
  };

  return (
    <div className="space-y-5">
      <div className="flex flex-wrap items-center gap-2">
        <Badge variant="primary">Rich Text</Badge>
        <Badge variant="neutral">Core widget</Badge>
        <Badge variant="neutral">Inline canvas editing</Badge>
      </div>

      <div className="text-sm text-muted-foreground">
        This widget is the friendly inline-authoring alternative to Markdown. In workspace edit mode
        users write directly on the card; settings remain for presentation and advanced HTML
        inspection.
      </div>

      <section className="space-y-3">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            <div className="text-sm font-medium text-topbar-foreground">HTML source</div>
            <p className="mt-1 text-sm text-muted-foreground">
              Advanced source view for inspection or direct HTML edits. Normal authoring should
              happen inline on the workspace card.
            </p>
          </div>
          <Button
            type="button"
            size="sm"
            variant="outline"
            disabled={!editable}
            onClick={() => {
              onDraftPropsChange({
                ...draftProps,
                contentHtml: richTextNoteStarterHtml,
              });
            }}
          >
            Use starter template
          </Button>
        </div>

        <Textarea
          value={draftProps.contentHtml ?? ""}
          readOnly={!editable}
          spellCheck={false}
          placeholder="<h1>Desk Note</h1><p>Write rich text HTML here if needed.</p>"
          className="min-h-[220px] font-mono text-xs leading-6"
          onChange={(event) => {
            onDraftPropsChange({
              ...draftProps,
              contentHtml: event.target.value,
            });
          }}
        />
      </section>

      <section className="grid gap-4 md:grid-cols-2">
        <label className="space-y-2">
          <span className="text-sm font-medium text-topbar-foreground">Content width</span>
          <Select
            value={contentWidth}
            disabled={!editable}
            onChange={(event) => {
              onDraftPropsChange({
                ...draftProps,
                contentWidth: normalizeRichTextNoteWidth(
                  event.target.value as RichTextNoteWidgetProps["contentWidth"],
                ),
              });
            }}
          >
            <option value="compact">Compact</option>
            <option value="prose">Prose</option>
            <option value="full">Full width</option>
          </Select>
        </label>

        <label className="space-y-2">
          <span className="text-sm font-medium text-topbar-foreground">Vertical alignment</span>
          <Select
            value={contentVerticalAlign}
            disabled={!editable}
            onChange={(event) => {
              onDraftPropsChange({
                ...draftProps,
                contentVerticalAlign: normalizeRichTextNoteVerticalAlign(
                  event.target.value as RichTextNoteWidgetProps["contentVerticalAlign"],
                ),
              });
            }}
          >
            <option value="top">Top</option>
            <option value="center">Center</option>
            <option value="bottom">Bottom</option>
          </Select>
        </label>

        <label className="space-y-2">
          <span className="text-sm font-medium text-topbar-foreground">Empty-state helper</span>
          <Input
            value={draftProps.emptyState ?? ""}
            readOnly={!editable}
            placeholder="Optional helper text when the note is empty"
            onChange={(event) => {
              const nextValue = event.target.value;

              onDraftPropsChange({
                ...draftProps,
                emptyState: nextValue.trim() ? nextValue : undefined,
              });
            }}
          />
        </label>
      </section>

      <section className="space-y-3">
        <div>
          <div className="text-sm font-medium text-topbar-foreground">Links</div>
          <p className="mt-1 text-sm text-muted-foreground">
            Choose whether links created in rich text open in a new tab or reuse the current tab.
          </p>
        </div>
        <div className="flex items-center gap-2">
          <Button
            type="button"
            size="sm"
            variant={openLinksInNewTab ? "default" : "outline"}
            disabled={!editable}
            onClick={() => {
              onDraftPropsChange({
                ...draftProps,
                openLinksInNewTab: true,
              });
            }}
          >
            New tab
          </Button>
          <Button
            type="button"
            size="sm"
            variant={!openLinksInNewTab ? "default" : "outline"}
            disabled={!editable}
            onClick={() => {
              onDraftPropsChange({
                ...draftProps,
                openLinksInNewTab: false,
              });
            }}
          >
            Same tab
          </Button>
        </div>
      </section>

      <section className="space-y-3">
        <div>
          <div className="text-sm font-medium text-topbar-foreground">Preview</div>
          <p className="mt-1 text-sm text-muted-foreground">
            Preview uses the same card renderer as the widget body. Inline editing activates only on
            the actual workspace canvas in edit mode.
          </p>
        </div>

        <div className="overflow-hidden rounded-[calc(var(--radius)-4px)] border border-border/70 bg-background/24">
          <div className="flex items-center gap-2 border-b border-border/70 px-3 py-2 text-xs uppercase tracking-[0.16em] text-muted-foreground">
            <FileText className="h-3.5 w-3.5" />
            Preview
          </div>
          <div className="max-h-[320px] min-h-[180px] overflow-auto">
            <RichTextNoteWidget
              widget={{
                id: "rich-text-note-preview",
                widgetVersion: "1.1.0",
                title: "Rich Text Preview",
                description: "Preview renderer",
                category: "Content",
                kind: "custom",
                source: "core",
                defaultSize: { ...DEFAULT_WIDGET_SIZE },
                canvasEditing: {
                  mode: "inline",
                },
                component: RichTextNoteWidget,
              }}
              props={previewProps}
            />
          </div>
        </div>
      </section>
    </div>
  );
}
