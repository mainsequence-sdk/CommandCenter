...
- `widget-settings.tsx`: shared settings trigger and modal that expose per-instance title and props editing, while rendering schema-driven settings sections plus any widget-specific advanced settings UI. It also exports the shared duplicate trigger used by workspace widget chrome.
