...
export const Textarea = React.forwardRef<HTMLTextAreaElement, TextareaProps>(
  (
    ...
    const [textareaRef, composedRef] = useComposedWidgetVariableReferenceRef(ref);
    const [referenceTokenEditing, setReferenceTokenEditing] = React.useState(false);
    const variableReference = useWidgetVariableReferenceTextField<HTMLTextAreaElement>({
      disabled,
      onChange,
    ...
    const editReferenceToken = (element: HTMLTextAreaElement) => {
    ...
    const textarea = (
      <textarea
        ...
        onKeyDown={(event) => {
          ...
          event.preventDefault();
          onWidgetReferenceCommit?.({
            value: textareaRef.current?.value ?? String(props.value ?? ""),
            reason: "enter",
          });
          setReferenceTokenEditing(false);
          return;
